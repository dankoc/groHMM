...
cells \citep{Core2008,Hah2011}.  GRO-seq, which provides a genome wide `map' 
...
occurring in cells \citep{Hah2011,Hah2013,Luo2014}.  
...
functions for counting raw reads \citep{PagesGRanges}, generating wiggle 
files for visualization \citep{Michael2009}, and creating metagene 
...
data from chromosome 7 only.  \Rpackage{groHMM} supports parallel processing 
and the number of cores to use can be set using \Rcode{mc.cores} option.
...
\Rpackage{GenomicFeatures} \citep{CarsonGFeatures} package.
...
calls `break up a single annotation') must be determined.   The optimal tuning 
...
Among the nine test cases, the sum of errors show minimal at case \#7, as shown
below.   \#4, however, is chosen over \#7 for further analysis due to the 
lower error rate per called transcript.  The variation of errors should be 
...
are used for either `run genes together' or  `break up a single annotation' 
...
of error by (1) breaking transcripts that have run together and 
(2) combining transcripts that have been broken.  The following method will 
generate a final set of transcripts for further analysis.
