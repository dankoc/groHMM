...
extern void* SSallocGamma(int num) {
  void* ss = (void*)calloc(1, sizeof(ssGamma)); // Assumed to be ssGamma*; // Cast to void* ??
...
extern void UpdateGamma(int state, void* ss, hmm_t *hmm) {
  ssGamma *SS = (ssGamma*)ss;
  double shape, scale;
  int updateRetVal= MLEGamma(SS[0].N, SS[0].sumPiXi, SS[0].sumLogPiXi, shape, scale, 1000, 10000);
  if(updateRetVal == 0) {
      hmm[0].em_args[state][0] = shape;
      hmm[0].em_args[state][1] = scale;
  }
  else {
...
extern void UpdateGamma_SHAPEeq1overSCALE(int state, void* ss, hmm_t *hmm) {
  ssGamma *SS = (ssGamma*)ss;
  MLEGamma_SHAPEeq1overSCALE(SS[0].N, SS[0].sumPiXi, SS[0].sumLogPiXi, SS[0].sumPiXiSq, hmm[0].em_args[state][0], hmm[0].em_args[state][1], 1000);
...
extern void UpdateGamma_SCALE1(int state, void* ss, hmm_t *hmm) {
  ssGamma *SS = (ssGamma*)ss;
  MLEGamma_SCALE1(SS[0].N, SS[0].sumPiXi, SS[0].sumLogPiXi, hmm[0].em_args[state][0], hmm[0].em_args[state][1], 1000);
...
extern void UpdateNormExp(int state, void* ss, hmm_t *hmm) {
  ...
  int MAXIT= 100;
  double tol=1e-3; // Don't have to be all that precise at each update ... so long as we are making headway.
  double *ex= SS[0].ex; 
  int *fncount=(int*)calloc(1,sizeof(int)), *grcount=(int*)calloc(1,sizeof(int));

  // Update mean.  Mean is the first entry in the **em_args matrix.
  cgmin(4, stateParams, sp, fmin, normal_exp_optimfn, normal_exp_optimgr, fail, tol, tol, ex, 1, 0, fncount, grcount, MAXIT);

  if(fail[0] != 0) {
    Rprintf("[UpdateNormExp] WARNING::Updates failed w/ message %d.  fncount= %d ; grcount= %d\n", fail[0], fncount[0], grcount[0]);
  }
  
  stateParams = sp;  // POSSIBLE MEMORY LEAK: Does this leak the old 'hmm[0].em_args[state]'??
  Rprintf("[UpdateNormExp]\t--> Alpha: %f; Mean: %f; Stdev: %f; Lambda: %f\n", hmm[0].em_args[state][0], hmm[0].em_args[state][1], hmm[0].em_args[state][2], hmm[0].em_args[state][3]);
}
...
extern void  TransUpdateP(int state, int nSequences, void* ss, hmm_t *hmm) {
  ...
  // Calculate Akl ==> i.e. the total times in state k; sum over other states, l' of: {k->l' transition}.
