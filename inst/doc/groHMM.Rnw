...
<<download refSeq genes, eval=TRUE>>=
...
<<tuning HMM, eval=FALSE, keep.source=TRUE, results=hide>>=
...
<<load Vignette data, eval=TRUE, echo=FALSE, results=hide>>=
load(system.file("extdata", "Vignette.RData", package="groHMM"))
@

<<HMM output, eval=TRUE, keep.source=TRUE>>=
tune
...
<<metagene analysis, eval=FALSE, keep.source=TRUE, results=hide>>=
upGenes <- rgChr7[de==1,]
...
save(tune, count0m, count40m, file="Vignette.RData")
@
