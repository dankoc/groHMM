...
Among these, GRO-seq provides information on instantaneous transactional responses 
...
transcriptional activity across the genome \emph{de novo} using a two-state hidden Markov 
...
include a time course of GRO-seq data following treatment with E2 (\emph{i.e.}, 0, 10, 40
...
from the UCSC database or alternatively, pre-made TranscriptDb objects can be used if 
...
Different isoforms of each gene are collapsed into one using the ENTREZID.  These 
...
nine test cases, the sum of errors show minimal at case \#7, as shown
below.   \#4, however, is chosen over \#7 for further analysis due to the lower error
...
\begin{enumerate}
  ...
  \item false transcript density (FTD)  = (5$'$ overhang area under the curve) /(max area for matched transcripts),
...
and genes either too small or too large in size are excluded.  And also size of transcripts and annotations are
...
some other alignable genomic features of interest).  It can be thought as a smoothed
